...
void autonomous(void) {
  ...
  motor_group backMotors = motor_group(L_Down, R_Down);
  drivetrain straight = drivetrain(L_Down, R_Down);

  //robot starts facing forward, turns 90 degrees to the right
  backMotors.spinFor(vex::forward, 0.83, seconds, 50, vex::velocityUnits::pct);
  //goes straight towards the red/blue mobile goal holding the teeter-totter down
  straight.drive(vex::forward, 10.0, vex::velocityUnits::rpm);
  //after this, it should pick up the mobile goal, move back slightly, and place it on the teeter-totter but I haven't gotten that far yet lmfao
}
...
void usercontrol(void) {
  ...
  while (1) {
    //  --- Main Robot Loop ---
    if (!gameMode){
      float vMove = Controller1.Axis4.position(percent) * velocityMult * 0.5;
      float hMove = Controller1.Axis3.position(percent) * velocityMult;
